...
RSpec.describe AdjustedMeow do
  it "creates a 0.25 second meow from a 0.741 second meow" do
    ...
    duration = 0.25
		...
		expect(adjust.duration_of_meow_in_seconds(meow)).to eq(0.741)
		# exepct(adjust.duration_of_meow_in_seconds(adjusted_meow)).to eq(0.25)
	end
  ...
  it "creates a 1.25 second meow from a 0.762 second meow" do
    ...
    expect(adjust.duration_of_meow_in_seconds(meow)).to eq(0.762)
	end
end
